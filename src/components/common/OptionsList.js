...
const OptionsList = (props) => {
  const { items, onItemSelected, isRightAligned, optionsWrapperCss } = props;
  ...
  const wrapperCss = useMemo(
    () => (isRightAligned ? styles["wrapper_right"] : styles["wrapper"]),
    [isRightAligned]
  );
  ...
  const renderItemCheckbox = (item) => {
    switch (item.isChecked) {
      case true:
        return <img src="assets/check16.png" alt="+" />;
      case false:
